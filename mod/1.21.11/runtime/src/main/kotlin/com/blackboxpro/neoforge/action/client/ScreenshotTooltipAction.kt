 ...
 * 2. ScreenRenderBridge 回调中用 renderTooltip() 将 tooltip 记录到 GuiRenderState
...
class ScreenshotTooltipAction : ActionExecutor {
    ...
    override fun execute(params: JsonObject, commandId: String): ActionResult {
        ...
        // 转换为 ClientTooltipComponent 列表
        val components = tooltipLines.map { ClientTooltipComponent.create(it.visualOrderText) }

        val mouseX = hoverState.mouseX.toInt()
        val mouseY = hoverState.mouseY.toInt()

        // 3. 通过 ScreenRenderBridge 注册一次性 afterRender 回调
        ...
        ScreenRenderBridge.registerOneShotAfterRender { drawContextAny ->
            ...
            try {
                // renderTooltip 直接写入 GuiRenderState，不走延迟机制
