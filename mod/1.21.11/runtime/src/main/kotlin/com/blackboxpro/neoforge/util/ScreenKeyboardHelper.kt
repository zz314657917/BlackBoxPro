...
object ScreenKeyboardHelper {
    ...
    private fun invokeScreenKeyPressed(screen: Any, keyCode: Int) {
        ...
        val keyEvent = createModernKeyEvent(keyCode)
        val method = screen.javaClass.methods.firstOrNull {
            it.name == "keyPressed" &&
                it.parameterTypes.size == 1 &&
                it.parameterTypes[0].name in MODERN_KEY_EVENT_TYPES
        } ?: throw IllegalStateException("Unsupported screen keyPressed signature")
        method.invoke(screen, keyEvent)
    }
    ...
    private fun invokeScreenCharTyped(screen: Any, char: Char) {
        ...
        val charEvent = createModernCharEvent(char)
        val method = screen.javaClass.methods.firstOrNull {
            it.name == "charTyped" &&
                it.parameterTypes.size == 1 &&
                it.parameterTypes[0].name in MODERN_CHAR_EVENT_TYPES
        } ?: throw IllegalStateException("Unsupported screen charTyped signature")
        method.invoke(screen, charEvent)
    }
    ...
    private fun createModernKeyEvent(keyCode: Int): Any {
        return runCatching {
            Class.forName("net.minecraft.client.input.KeyEvent")
                .getConstructor(java.lang.Integer.TYPE, java.lang.Integer.TYPE, java.lang.Integer.TYPE)
                .newInstance(keyCode, 0, 0)
        }.getOrElse {
            Class.forName("net.minecraft.client.input.KeyInput")
                .getConstructor(java.lang.Integer.TYPE, java.lang.Integer.TYPE, java.lang.Integer.TYPE)
                .newInstance(keyCode, 0, 0)
        }
    }

    private fun createModernCharEvent(char: Char): Any {
        return runCatching {
            Class.forName("net.minecraft.client.input.CharacterEvent")
                .getConstructor(java.lang.Integer.TYPE, java.lang.Integer.TYPE)
                .newInstance(char.code, 0)
        }.getOrElse {
            Class.forName("net.minecraft.client.input.CharInput")
                .getConstructor(java.lang.Integer.TYPE, java.lang.Integer.TYPE)
                .newInstance(char.code, 0)
        }
    }

    private fun inputKeyForKeyCode(keyCode: Int): InputConstants.Key {
        return runCatching {
            InputConstants::class.java
                .getMethod("getKey", java.lang.Integer.TYPE, java.lang.Integer.TYPE)
                .invoke(null, keyCode, 0) as InputConstants.Key
        }.getOrElse {
            val keyEventClass = Class.forName("net.minecraft.client.input.KeyEvent")
            val keyEvent = keyEventClass
                .getConstructor(java.lang.Integer.TYPE, java.lang.Integer.TYPE, java.lang.Integer.TYPE)
                .newInstance(keyCode, 0, 0)
            InputConstants::class.java
                .getMethod("getKey", keyEventClass)
                .invoke(null, keyEvent) as InputConstants.Key
        }
    }

    private val MODERN_KEY_EVENT_TYPES = setOf(
        "net.minecraft.client.input.KeyEvent",
        "net.minecraft.client.input.KeyInput"
    )

    private val MODERN_CHAR_EVENT_TYPES = setOf(
