...
object ContainerTooltipHelper {
    ...
    data class TooltipSnapshot(
        ...
        fun toSlotTooltipJson(): JsonObject = JsonObject().apply {
            ...
            if (!empty) {
                ...
                add("tooltipJson", JsonArray().apply { tooltipJson.forEach { add(it) } })
            }
        }
    }
    ...
    fun querySlotTooltip(slotIndex: Int, advanced: Boolean = false): TooltipSnapshot {
        ...
        if (slotIndex !in 0 until menu.slots.size) {
            throw IllegalArgumentException("Slot index $slotIndex out of range [0, ${menu.slots.size})")
        }

        return createTooltipSnapshot(client, slotIndex, menu.slots[slotIndex].item, advanced)
    }
    ...
    fun queryCurrentTooltip(advanced: Boolean = false): TooltipState {
        ...
        val hovered = findHoveredSlot(screen.menu.slots, layout, mouse.first, mouse.second)
            ?: return TooltipState(visible = false, mouseX = mouse.first, mouseY = mouse.second)
        val snapshot = createTooltipSnapshot(client, hovered.first, hovered.second.item, advanced)
        return TooltipState(
    ...
    fun hoverSlot(windowId: Int, slotIndex: Int, advanced: Boolean = false): TooltipState {
        ...
        if (menu.containerId != windowId) {
            throw IllegalArgumentException("Current windowId ${menu.containerId} does not match requested $windowId")
        }
        if (slotIndex !in 0 until menu.slots.size) {
            throw IllegalArgumentException("Slot index $slotIndex out of range [0, ${menu.slots.size})")
        }
        ...
        val snapshot = createTooltipSnapshot(client, slotIndex, slot.item, advanced)
        return TooltipState(
            visible = !snapshot.empty,
    ...
    private fun currentGuiMouse(client: Minecraft): Pair<Double, Double>? {
    ...
    private fun moveCursor(client: Minecraft, guiX: Double, guiY: Double) {
    ...
    private fun resolveLayout(screen: AbstractContainerScreen<*>): ScreenLayout? {
        val left = readInt(screen, "leftPos", "x") ?: return null
        val top = readInt(screen, "topPos", "y") ?: return null
